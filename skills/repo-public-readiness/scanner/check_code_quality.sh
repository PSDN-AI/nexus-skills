...
if command -v shellcheck &>/dev/null; then
  while IFS= read -r -d '' f; do
    while IFS= read -r sc_line; do
      ln=$(echo "$sc_line" | jq -r '.line // "-"' 2>/dev/null || echo "-")
      level=$(echo "$sc_line" | jq -r '.level // "warning"' 2>/dev/null || echo "warning")
      msg=$(echo "$sc_line" | jq -r '.message // "shellcheck issue"' 2>/dev/null || echo "shellcheck issue")
      code=$(echo "$sc_line" | jq -r '.code // ""' 2>/dev/null || echo "")
      sev="MEDIUM"
...
if [[ -f "$REPO_PATH/package.json" ]]; then
  if command -v npm &>/dev/null; then
    ...
    vuln_count=$(echo "$audit_output" | jq '.metadata.vulnerabilities.high // 0' 2>/dev/null || echo "0")
    crit_count=$(echo "$audit_output" | jq '.metadata.vulnerabilities.critical // 0' 2>/dev/null || echo "0")
...
if [[ -f "$REPO_PATH/requirements.txt" ]]; then
  if command -v pip-audit &>/dev/null; then
    ...
    vuln_count=$(echo "$vuln_output" | jq 'length' 2>/dev/null || echo "0")
...
if command -v trivy &>/dev/null; then
  ...
  vuln_count=$(echo "$trivy_output" | jq '[.Results[]?.Vulnerabilities // [] | length] | add // 0' 2>/dev/null || echo "0")
  if [[ "$vuln_count" -gt 0 ]]; then
