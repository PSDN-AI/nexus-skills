...
Exit codes: `0` success | `1` missing input | `2` invalid tasks.yaml or repo | `3` workspace failure | `4` task execution failure | `5` merge failure | `6` internal error
...
3. If `--resume` is set, load prior `run-report.yaml` and restore task statuses.
...
3. Mark tasks as `blocked` if any dependency has `failed` status.
4. Respect `--max-parallel` concurrency limit.

### Phase 4: PREPARE

1. Create integration branch: `agent-launcher/{domain}/{execution-id}` from base branch.
2. Create per-task branches: `agent/{task-id-lower}` from the integration branch.
3. Write prompt files for each task at `.agent-launcher/{task-id}.prompt.md`.
...
2. Check parallel safety (no `files_touched` overlap with concurrent tasks).
3. Switch to the task branch and invoke the sub-agent with the task's `prompt_context`.
4. The sub-agent commits changes to the task branch.
...
3. If a merge conflict occurs, abort and mark the task as `failed`.
...
6. **Every task run must leave a structured status trail** in `run-report.yaml`.
7. **Stop retrying when policy is exhausted**; surface a precise handoff instead of looping.
8. **Respect `agent_model` and `max_iterations`** from `config.yaml`.
...
- Blocked tasks trace back to a specific failed dependency.
...
- **Resume with modified tasks.yaml**: If you change tasks.yaml between runs, resume may skip tasks that should re-run. Clear the run report to force a fresh start.
